...
`message` ツールの `read` アクションではスレッド返信を取得できないため、Slack Web APIを直接使用。

### 方法
```bash
curl -s -H "Authorization: Bearer ${SLACK_BOT_TOKEN}" \
  "https://slack.com/api/conversations.replies?channel=CHANNEL_ID&ts=MESSAGE_TS&limit=50"
```

### URLからの変換
- SlackスレッドURL: `https://waka-xit3422.slack.com/archives/C052GSRK97C/p1771039590662119`
- Channel ID: `C052GSRK97C`
- Message TS: `1771039590.662119` (pの後の数字を小数点で区切る)

### 整形して読む
```bash
curl -s -H "Authorization: Bearer ${SLACK_BOT_TOKEN}" \
  "https://slack.com/api/conversations.replies?channel=C052GSRK97C&ts=1771039590.662119&limit=50" \
  | jq -r '.messages[] | "\n---\n[\(.ts)] \(.user // .username // "bot"):\n\(.text)\n"'
```
